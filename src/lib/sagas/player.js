...
import { effects, eventChannel, delay } from 'redux-saga';
...
import {
    PLAYER_STATUS
} from '../reducer/player.js';

const { takeLatest, put, call, select, take, race, fork } = effects;
...
export function* playNextSongWorker() {
    const { playlist, next } = yield select(state => state.player);
...
export function* updatePlayStateWorker() {
    try {
        ...
        console.error('Unable to get playState of wx background audio player');
        console.error(err);
    }
}
